...
* "This submission is for the following group: Anthony Chan(alc1), Minwhan cho(minwhanc), Zhichuan Duan(zhichuad), Devansh Kukreja(dkukreja)
...
ggraph() functions similarly to ggplot() in initializing a plot
...
ggraph(LA_graph, layout = 'kk') +
  geom_edge_link() +
  geom_node_label(aes(label = actor_names), size = 4) +
  theme_graph()
```
...
ggraph(LA_graph_weighted, layout = 'circle') +
  geom_edge_link(aes(edge_width = weight)) +
  geom_node_label(aes(label = actor_names), size = 2.8) +
...
The earliest bike trip was June 1st and the latest was June 30th
...
ggplot(trips_per_day_usertype, aes(x = start_date, y = n_trips, color = usertype)) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Bike Trips Per Day By Usertype",
    x = "Date",
    y = "Number of Trips"
  )
```
...
It appears that customers have more fluctuation which makes sense since customers tend to use bikes in response to events/circumstances rather than on some sort of schedule.
...
ggplot(trips_per_day_usertype, aes(x = start_date, y = n_trips, color = usertype)) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Bike Trips Per Day By Usertype with Rolling Average",
    x = "Date",
    y = "Number of Trips"
  )
```
...
The customer moving average appears to have more fluctuation.
...
+ yes
+ With lag 0, the correlation is 1
+ This makes sense since the data is just being compared to itself and data correlated with itself always has correlation 1.
...
The correlations at 24 and 28 are marginally below the blue line indicating a statistically significant difference. This doesn't make sense as in theory there should be no patterns but this probably happened as a result of the random nature of the datga.
...
ggplot(bike_customer_acf_df, aes(x = lag, y= acf)) + 
  ...
  labs(
    y = "Autocorrelation",
    title = "Autocorrelation in Subscribers",
    x = "lag(days)",
    caption = "source: CitiBike TripHistories Data Set"
  )
```
...
```{r}
bike_subsciber_acf_df <- with(bike_subsciber_acf, data.frame(lag, acf))
ggplot(bike_subsciber_acf_df, aes(x = lag, y = acf)) +
  geom_col() +
  labs(
    title = "Autocorrelation in Customers",
    y = "Autocorrelation",
    x = "Lag(days)",
    caption = "Source: CitBike Trip Histories Data Set"
  )
```
...
lag=7 is a lag period of 7 days which indicates to us that bike usage runs on a weekly cycle and so on the same day of the week users tend to follow the same patterns.
